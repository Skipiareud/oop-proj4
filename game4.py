...
class Player:
    ...
    def __init__(self, name: str, x: int, keys: List[int], color: Tuple[int, int, int]):
        ...
        # 키 프레스 효과용
        self.key_press_effects = [0.0] * self.lanes  # 각 레인의 효과 강도 (0.0 ~ 1.0)
    ...
    def reset(self):
        ...
        self.pressed_lanes = [False] * self.lanes
        self.ready = False
        self.key_press_effects = [0.0] * self.lanes
    ...
    def update(self, dt: float):
        ...
        for i in range(self.lanes):
            if self.key_press_effects[i] > 0:
                self.key_press_effects[i] = max(0, self.key_press_effects[i] - dt * 4.0)
    
    def handle_key_press(self, lane: int):
        """키 입력 처리"""
        self.pressed_lanes[lane] = True
        self.key_press_effects[lane] = 1.0  # 효과 최대치
    ...
    def draw(self, screen: pygame.Surface):
        ...
        pygame.draw.line(screen, RED, 
                        ...
                        (self.x + self.lane_width * self.lanes, self.judgment_line_y), 6)
        
        # 노트 그리기
        for note in self.notes:
            note.draw(screen, self.x)
        
        # 키 프레스 효과 - 판정선 아래에서 위로 올라가는 효과
        for i in range(self.lanes):
            if self.key_press_effects[i] > 0:
                effect_strength = self.key_press_effects[i]
                
                # 반투명 흰색 효과
                alpha = int(200 * effect_strength)
                effect_surface = pygame.Surface((self.lane_width - 4, 200), pygame.SRCALPHA)
                
                # 그라데이션 효과 (아래에서 위로)
                for y in range(200):
                    gradient_alpha = int(alpha * (1 - y / 200) * effect_strength)
                    color_with_alpha = (255, 255, 255, gradient_alpha)
                    pygame.draw.line(effect_surface, color_with_alpha, 
                                   (0, y), (self.lane_width - 4, y))
                
                # 효과 그리기 (판정선 아래부터)
                screen.blit(effect_surface, 
                          (self.x + i * self.lane_width + 2, 
                           self.judgment_line_y - 200))
        
        # 키가 눌려있을 때 레인 전체 밝게 표시
        for i, pressed in enumerate(self.pressed_lanes):
            if pressed:
                # 레인 전체 은은하게 밝게
                lane_surface = pygame.Surface((self.lane_width - 4, SCREEN_HEIGHT), pygame.SRCALPHA)
                lane_surface.fill((255, 255, 255, 30))
                screen.blit(lane_surface, (self.x + i * self.lane_width + 2, 0))
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("\n필요한 라이브러리:")
    print("  pip install pygame librosa soundfile numpy")
    print("\n사용 방법:")
    print("  'music' 폴더에 MP3/WAV 파일을 넣기 ㄱㄴ")
    print("  1. ↑↓ 키로 음악을 선택하고 ENTER로 확인")
    print("  2. 1p/2p가 준비되면 게임 시작!")
    print("=" * 60)
    print()
